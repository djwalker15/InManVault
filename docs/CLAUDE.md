...
**Catalog:**
- `products` — specific purchasable product (name, brand, variant, barcode, image, size). `variant` is a nullable free-text descriptor (flavor/scent/style — "Lime", "Zero Sugar Cherry") with autocomplete from prior values; size is separate (`size_value`/`size_unit`). Has `product_group_id` (FK → ProductGroup, nullable) linking to a generic group. Has `source` field tracking origin. Has `is_package boolean` (app-maintained: true iff active `product_components` rows exist). Nullable `crew_id`.
...
- **Checking Stock** — Search-first with real-time filtering. Browse by Space or Category. Three filters (category, space, stock status). Inline expansion with detail + recent activity. Nine inline actions (restock, use, move, set home, put back, log waste, add to list, edit, adjust count). Alerts summary with dashboard widget.
